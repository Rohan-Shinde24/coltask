...
import { registerSchema as schema } from '@/lib/validations';
    ...
    <div className="min-h-screen flex flex-col md:flex-row bg-[#fdfdfc] text-black font-sans relative">
      ...
      <div className="hidden md:flex md:w-1/2 bg-black text-white p-12 flex-col justify-between relative overflow-hidden">
        ...
        <div className="relative z-10 mb-20">
          <h1 className="text-5xl font-heading font-bold mb-4">Start your workspace.</h1>
          <p className="text-xl text-white/60">Join Coltask today and start managing your projects securely.</p>
        </div>
      </div>

      {/* Right Form Side */}
      <div className="w-full md:w-1/2 flex items-center justify-center p-6 sm:p-12 relative min-h-screen md:min-h-0 py-12 md:py-8">
        ...
        <div className="w-full max-w-md bg-white rounded-3xl shadow-2xl border border-black/10 relative z-10 overflow-hidden md:shadow-none md:border-none md:bg-transparent">
          <div className="p-8 sm:p-12 md:p-0">
            ...
            <h2 className="text-3xl font-heading font-bold text-center md:text-left mb-8">Create an Account</h2>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
              <div>
                ...
                <input
                  ...
                  className={`w-full px-4 py-3 bg-black/5 border border-transparent rounded-xl focus:bg-white focus:border-black focus:ring-1 focus:ring-black outline-none transition-all ${errors.name ? 'border-red-500 focus:border-red-500 focus:ring-red-500' : ''}`}
                  {...register('name')}
                />
              ...
              <div className="pt-2">
                <label className="flex items-start gap-3 cursor-pointer group">
                  <input 
                    type="checkbox" 
                    className={`mt-1 w-4 h-4 rounded border-black/20 text-black focus:ring-black ${errors.terms ? 'border-red-500' : ''}`}
                    {...register('terms')}
                  />
                  <span className="text-sm font-medium text-black/70">
                    I agree to the <Link href="/terms" className="text-black font-semibold hover:underline">Terms & Conditions</Link> and <Link href="/privacy" className="text-black font-semibold hover:underline">Privacy Policy</Link>
                  </span>
                </label>
                {errors.terms && <span className="text-red-500 text-xs mt-1 block pl-7">{errors.terms.message}</span>}
              </div>

              <button 
                type="submit" 
                className="w-full bg-black text-white py-3.5 rounded-xl font-medium hover:bg-black/80 transition-colors flex items-center justify-center mt-4 disabled:opacity-70 disabled:cursor-not-allowed"
